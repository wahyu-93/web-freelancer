                                ...
                                <tbody class="bg-white">
                                    @foreach ($services as $service)
                                        <tr class="text-gray-700 border-b">
                                                ...
                                                <div class="flex items-center text-sm">
                                                    <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                        ...
                                                        @if(count($service->thumbnails))
                                                            <img class="object-cover w-full h-full rounded" src="{{ Storage::url($service->thumbnails[0]->thumbnail) }}" alt="" loading="lazy" />
                                                        @else
                                                            <svg class="w-full h-full object-cover rounded" fill="currentColor" viewBox="0 0 24 24">
                                                    ...
                                                    <div>
                                                        <a href="{{ route('member.service.show', 1) }}" class="font-medium text-black">
                                                            {{ $service->title }} <br> {{ $service->description }}
                                                        </a>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-1 py-5 text-sm">
                                                <a href="{{ route('member.service.edit', $service) }}" class="px-4 py-2 mt-2 text-left text-white rounded-xl bg-serv-email">
                                                    Edit Service
                                                </a>
                                            </td>
                                        </tr>
