            ...
            <div class="grid w-full gap-5 px-10 mx-auto md:grid-cols-12">
                <div class="col-span-8">
                    ...
                    <p class="text-sm text-gray-400">
                        {{ auth()->user()->orderbuyers()->count() }} Total Requests
                    </p>
                </div>
                    ...
                    <div class="px-6 py-2 mt-2 bg-white rounded-xl">
                                        ...
                                        <div class="flex items-center text-sm">
                                            <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                ...
                                                @if($order->userFreelancer()->first()->detailUser->photo != NULL)
                                                    <img class="object-cover w-full h-full rounded-full" src="{{ Storage::url($order->userFreelancer()->first()->detailUser->photo) }}" alt="" loading="lazy" />
                                                @else
                                            ...
                                            <div>
                                                <p class="font-medium text-black">{{ $order->userFreelancer->name }}</p>
                                                <p class="text-sm text-gray-400">{{ $order->userFreelancer()->first()->detailUser->contact_number }}</p>
                                            </div>
                                        </div>
                                    </td>
                        ...
                        <div class="">
                            ...
                            <div class="px-1 py-4 text-right">
                                ...
                                @if(isset($order->file) && $order->order_status_id == 2)
                                    <a href="{{ route('member.approve.request', $order->id) }}" type="submit" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                        Approve
                                    </a>
                                @else
                                    <button class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                        Approve
                                    </button>
                                @endif
