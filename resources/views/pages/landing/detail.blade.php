            ...
            <main class="p-4 lg:col-span-8 md:col-span-12">
                ...
                @if(count($thumbnails))
                    <div class="p-3 my-4 bg-gray-100 rounded-lg image-gallery" x-data="gallery()">
                        <input type="hidden" id="firstPhoto" name="firstPhoto" value="{{ Storage::url($thumbnails[0]->thumbnail) }}">
                        <img :src="featured" alt="" class="rounded-lg cursor-pointer w-full h-128" data-lity>
                        <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                            <div class="flex mt-2 flex-nowrap">
                                @forelse ($thumbnails as $thumbnail)
                                    <img 
                                        :class="{'border-4 border-serv-button': active === {{ $thumbnail->id }} }" 
                                        @click="changeThumbnail('{{ Storage::url($thumbnail->thumbnail) }}', {{ $thumbnail->id }})" 
                                        src="{{ Storage::url($thumbnail->thumbnail) }}" 
                                        alt="thumb gallery" class="inline-block mr-2 rounded-lg cursor-pointer w-36 h-20">
                                @empty
                                    <p class="text-g italic">Thumbail is not yet available</p>
                                @endforelse
                    ...
                    <div x-data="{ tab: window.location.hash ? window.location.hash.substring(1) : 'description' }" id="tab_wrapper">
                        ...
                        <div x-show.transition.duration.500ms="tab === 'description'" class="leading-8 text-md">
                            ...
                            <ul class="mb-4 list-check">
                                @forelse ($advantageServices as $advantageService)
                                    <li class="pl-10 my-2">{{ $advantageService->advantage }}</li>
                                @empty
                                    <p class="text-g italic">Advantage Service is not yet available</p>
                                @endforelse
                            </ul>
                            <p class="mb-4">
                                {{ $service->note ?? '' }}
                        ...
                        <div x-show.transition.duration.500ms="tab === 'seller'" class="leading-8 text-md">
                                ...
                                <div class="flex items-center col-span-12 p-2 lg:col-span-6">
                                    <div class="flex items-center space-x-4">
                                        @if($service->user->detailUser->photo != NULL)
                                            <img 
                                                src="{{ Storage::url($service->user->detailUser->photo) }}" alt="myProfile"
                                                class="w-20 h-20 rounded-full">
                                        @else
                                            <svg class="w-20 h-20 rounded-full text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="mt-4 mb-8 content-description">
                                <p>
                                    {{ $service->user->detailUser->biografi }}
                                </p>
                            </div>
                            <h3 class="my-4 text-lg font-semibold">My Experiences</h3>
                            <ul class="mb-8 list-check">
                                @forelse ($service->user->detailUser->experienceUsers as $experience)
                                    <li class="pl-10 my-2">{{ $experience->experience }}</li>
                                @empty
                                    <p class="text-g italic">Experience is not yet available</p>
                                @endforelse
                            </ul>
                            <h3 class="my-4 text-lg font-semibold">Skills</h3>
                            <div class="mb-8 skills">
                                @forelse ($taglines as $tagline)
                                    <span class="inline-block px-3 py-1 mr-2 rounded bg-serv-services-bg">{{ $tagline->tagline }}</span>
                                @empty
                                    <p class="text-g italic">Tagline is not yet available</p>
                                @endforelse
                            </div>
                            <hr class="border-serv-services-bg">
                            <p class="my-4 text-lg text-gray-400">
                                Joined Since {{ date("d F Y", strtotime($service->user->created_at)) }}
                            </p>
                        </div>
                ...
                <div class="mb-4 border rounded-lg border-serv-testimonial-border">
                    ...
                    <div class="flex items-center px-2 py-3 mx-4 mt-4 border rounded-full border-serv-testimonial-border">
                        ...
                        <div class="flex-1 text-sm font-medium text-center">
                            ...
                            {{ $service->revision_time }} Revision Limit
                        </div>
                    </div>
                    ...
                    <div class="px-4 pb-4 booking">
                        @auth
                            <a href="{{ route('booking.landing', $service) }}" class="block px-12 py-4 my-2 text-lg font-semibold text-center text-white bg-serv-button rounded-xl">
                                Booking Now
                            </a>
                        @endauth
...
@push('after-script')
    <script>
        function gallery(){
            let firstPhoto = document.getElementById('firstPhoto').value; 
            return {
                featured: firstPhoto,
                active: 1,
